sap.ui.define([
	...
	function getFrameUrl (sHash, sUrlParameters) {
		var sUrl = jQuery.sap.getResourcePath("jetCourses/MasterDetailApp/app", ".html");
		sHash = sHash || "";
		sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

		if (sHash) {
			sHash = "#MasterDetailApp-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
		} else {
			sHash = "#MasterDetailApp-display";
		}

		return sUrl + sUrlParameters + sHash;
	}

	return Opa5.extend("jetCourses.MasterDetailApp.test.integration.pages.Common", {
		...
		getMockServer : function () {
			return new Promise(function (success) {
				Opa5.getWindow().sap.ui.require(["jetCourses/MasterDetailApp/localService/mockserver"], function (mockserver) {
