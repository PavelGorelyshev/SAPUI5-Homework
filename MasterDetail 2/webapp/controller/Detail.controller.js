	...
	return BaseController.extend("jetCourses.MasterDetailApp.controller.Detail", {
		...
		onPressDeactivateDelete: function(oEvent) {
			...
			if (this.getModel("detailView").getProperty("/button/pressed/ChangeVersionMode")) {
				...
				MessageBox.confirm(this.getResourceBundle().getText(this.getResourceBundle().getText("msgDeactivate")), {
					onClose: oAction => {
						if (oAction === MessageBox.Action.OK) {
							aSelectedContexts.forEach(oContext => {
		...
		onPressCopy: function(oEvent) {
			let oGetCopyContext = this._oTable.getSelectedIndices()[0];
			let oGetObjectContext = this._oTable.getContextByIndex(oGetCopyContext).getObject();
			sap.ui.core.Fragment.load({
				name: "jetCourses.MasterDetailApp.view.CreateGroup",
				controller: this
			})
			.then(oDialog => {
				let oContext = this.getModel().createEntry(this._oSmartTable.getEntitySet(), {
					properties: {
						GroupID: "",
						Version: oGetObjectContext.Version,
						Language: "EN",
						GroupText: oGetObjectContext.GroupText
					}
				});
